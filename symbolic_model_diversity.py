...
def load_diversity_scores(file_path):
    """Load diversity scores from a CSV file."""
    df = pd.read_csv('C:/Users/utkid/Prediss_Chest/Hands-On/data/ml1m/preprocessed/cafe/tmp/diversity_scores_1.csv')
    diversity_scores = df.set_index('User_ID')['Diversity_Score'].to_dict()
    return diversity_scores

class EntityEmbeddingModel(nn.Module):
    def __init__(self, entity_info, embed_size, init_embed=None, diversity_scores=None):
...
class SymbolicNetwork(nn.Module):
    print("yess!!!!!")
    def __init__(self, relation_info, embedding, deep_module, use_dropout, device, diversity_scores=None):
    ...
    def forward(self, metapath, pos_paths, neg_pids):
        #print("forward method is being called")
        """Compute loss.
        Args:
        ...
        user_diversity_scores = torch.tensor([self.diversity_scores.get(uid.item(), 0) for uid in pos_paths[:, 0]], device=self.device)
        
        user_diversity_scores_tensor = user_diversity_scores.clone().detach().view(-1, 1).to(self.device)

        outputs = [torch.cat([output, user_diversity_scores_tensor], dim=1) for output in outputs]
        outputs_without_diversity_scores = [output[:, :-1] for output in outputs]
        ...
        for i, module in enumerate(modules):
            ...
            scores = torch.matmul(outputs_without_diversity_scores[i], et_vecs.t())
        ...
        reg_loss = reg_loss * user_diversity_scores
        rank_loss = rank_loss * user_diversity_scores
...
def create_symbolic_model(args, kg, train=True, pretrain_embeds=None):
    ...
    entity_embed_model = EntityEmbeddingModel(entity_info, args.embed_size, init_embed=pretrain_embeds, diversity_scores=diversity_scores)
    model = SymbolicNetwork(relation_info, entity_embed_model, args.deep_module, args.use_dropout, args.device, diversity_scores=diversity_scores)
