...
from symbolic_model_diversity import SymbolicNetwork, create_symbolic_model, load_diversity_scores
...
# Load diversity scores at the beginning of your script
diversity_scores = load_diversity_scores('../../data/ml1m/preprocessed/cafe/tmp/diversity_scores_1.csv')

max_diversity_score = max(diversity_scores.values())
min_diversity_score = min(diversity_scores.values())
for key in diversity_scores:
    diversity_scores[key] = (diversity_scores[key] - min_diversity_score) / (max_diversity_score - min_diversity_score)
...
def dcg_at_k(r, k, method=1, weights = None):
    ...
    """
    r = np.asfarray(r)[:k]
    weights = np.asfarray(weights)[:k] if weights else np.ones_like(r)
    
    if r.size:
        if method == 0:
            return np.sum(r[0] * weights[0] + np.sum(r[1:] / np.log2(np.arange(2, r.size + 1)) * weights[1:]))
        elif method == 1:
            return np.sum(r / np.log2(np.arange(2, r.size + 2)) * weights)
        else:
...
def ndcg_at_k(r, k, method=1, weights = None):
...
def evaluate_with_insufficient_pred(topk_matches, test_user_products):
    ...
    """
    # Compute metrics
    precisions, recalls, ndcgs, hits, our_ndcgs, isolation_indexes, coverages, w_ndcgs = [], [], [], [], [], [], [], []
    ...
    for uid in test_user_idxs:
        ...
        while len(pred_list) < 50:
            pred_list.append(0)
        rel_set = test_user_products[uid]
        total_unique_items.update(rel_set)

        dcg = 0.0
        w_dcg = 0.0  # Initialize weighted DCG
        hit_num = 0.0
        hits = []
        for i in range(len(pred_list)):
            if pred_list[i] in rel_set:
                dcg += 1. / (math.log(i + 2) / math.log(2))
                w_dcg += 1. / (math.log(i + 2) / math.log(2)) * some_weight_function(i)  # Calculate weighted DCG
                hit_num += 1
                hits.append(1)
            else:
                hits.append(0)
        ...
        # idcg
        idcg = 0.0
        w_idcg = 0.0  # Initialize weighted IDCG
        for i in range(min(len(rel_set), len(pred_list))):
            idcg += 1. / (math.log(i + 2) / math.log(2))
            w_idcg += 1. / (math.log(i + 2) / math.log(2)) * some_weight_function(i)  # Calculate weighted IDCG
        ndcg = dcg / idcg
        w_ndcg = w_dcg / w_idcg  # Calculate weighted NDCG
        ...
        # Isolation Index
        unique_recommendations = set(pred_list)
        isolation_index = len(unique_recommendations - set(rel_set)) / len(unique_recommendations)
        isolation_indexes.append(isolation_index)

        # Update the set of all recommended items
        all_recommended_items.update(unique_recommendations)
        ...
        hits.append(hit)
        w_ndcgs.append(w_ndcg)  # Append weighted NDCG to the list

    # Coverage
    coverage = len(all_recommended_items) / len(total_unique_items)
    ...
    avg_isolation_index = np.mean(isolation_indexes)
    avg_coverage = np.mean(coverages)
    avg_w_ndcg = np.mean(w_ndcgs)  # Calculate average weighted NDCG

    msg = 'NDCG={:.3f} | Recall={:.3f} | HR={:.3f} | Precision={:.3f} | Isolation Index={:.3f} | Coverage={:.3f} | wNDCG={:.3f}'.format(
        avg_ndcg, avg_recall, avg_hit, avg_precision, avg_isolation_index, avg_coverage, avg_w_ndcg)
    print(msg)
    return msg

def some_weight_function(i):
    # Exponential decay function to give higher weight to top-ranked items
    return math.exp(-0.05 * i)
    ...
    def execute(self, program, uid, excluded_pids=None):
        ...
        while queue:  # queue is not empty
            ...
            for path, value, ep, mp in node.parent.data['paths']:
                ...
                # Use self.diversity_scores to influence the model's behavior
                valid_scores =alpha* valid_scores + (1-alpha)* self.diversity_scores.get(uid, 1.0)
...
def create_heuristic_program(metapaths, raw_paths_with_scores, prior_count, sample_size):
    ...
    for mpid, metapath in enumerate(metapaths):
        
        user_id, relation= metapath[0]
        diversity_score = diversity_scores.get((user_id, relation), 1.0)
        norm_count[mpid] *= diversity_score
...
def run_program(args):
    ...
    for uid in test_labels:
     #   diversity_score= diversity_scores.get(uid,1.0)
      #  if diversity_score > 3.5:  # You need to define SOME_THRESHOLD
       #     diversity_score *= 0.9  # You need to define ADJUSTMENT_FACTOR
        ...
        pred_labels[uid] = [t[0] for t in tmp] #if t[1]>=diversity_score
        pbar.update(1)
    with open("C:/Users/utkid/Prediss_Chest/Hands-On/results/ml1m/cafe" + "/red_labels.pkl", 'wb') as pred_paths_file:
