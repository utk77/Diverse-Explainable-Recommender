...
def entity2plain_text(dataset, method):
    ...
    if method == "cafe":
        with gzip.open(f"../../data/{dataset}/preprocessed/{method}/kg_entities.txt.gz", 'rt', encoding='utf-8') as entities_file:
        ...
        with gzip.open(f"../../data/{dataset}/preprocessed/{method}/mappings.txt.gz", 'rt', encoding='utf-8') as entities_file:
            reader = csv.reader(entities_file, delimiter="\t")
            next(reader, None)
            for row in reader:
                row[0] = row[0].split("_")
